...
**Backups** are copies of data or systems created for:
...
**Scenario:** A developer accidentally runs a migration script that corrupts product catalog data in BharatMart database.
...
**Scenario:** BharatMart application logs stored on a block volume are accidentally deleted, and logs are needed for incident investigation.
...
**Scenario:** Security audit requires rotating database password for BharatMart. Current password is stored in configuration files.
